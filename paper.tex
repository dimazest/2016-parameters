...
We evaluate these heuristics by comparing the performance they give on SimLex-999 against that obtained using the best possible parameter selections (determined via an exhaustive search for each dimensionality choice); and the best models reported by \cite{TACL570} (see Figure~\ref{fig:global-best-simlex}).
...
Finally, to see whether the heuristics transfer robustly, we repeat this comparison on the MEN dataset (see Figure~\ref{fig:global-best-men}). Again, PMI and CPMI follow the best possible setup and there is a little gap between the chosen models and the best possible selection for SPMI and SCPMI.
